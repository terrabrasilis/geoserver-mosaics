...
if (length(args)==2) {
  ...
  # shape <-  "/home/adeline/Dropbox/github_projects/geoserver-mosaics/shapefiles/limite_pampa/grid_landsat_tm_Pampa_crop_4326_newIBGE_v2.shp"
  cat("shapefile path: '", shape, "' \n")
  rasterFolder <- as.character(args[2])
  # rasterFolder <- "/home/adeline/Downloads/prodes/biomas/pampa/temp8bits/tempNoData0/tempEPSG4326"
  ...
  # stringi::stri_sub(pathrowShp, 4, 3) <- 0

  for (i in 1:length(myRasters)){
    #i=3
    filename <- basename(tools::file_path_sans_ext(myRasters[i]))
    pattern_landsat1 <- "[[:digit:]]{5}[[:punct:]]{1}" #"[[:punct:]]{1}[[:digit:]]{5}[[:punct:]]{1}" <- CHANGE PATTERN OF FILENAME
    ...
    if(isFALSE(rlang::is_empty(pathrowRasterValue))){
      ...
      pattern_landsat <- "[[:digit:]]{5}[[:punct:]]{1}" #"[[:punct:]]{1}[[:digit:]]{5}[[:punct:]]{1}" <- CHANGE PATTERN OF FILENAME
      exist_pathrow <- sub("_","",regmatches(filename, regexpr(pattern_landsat, filename)))
      #exist_pathrow <- sub(sub('0(.{2})$', '\\1', exist_pathrow1),"", regmatches(filename, regexpr(pattern_landsat, filename)))
    ...
    if(isTRUE(file.exists(paste(rasterFolder, newFolder, "/", filename, "_cutted.tif", sep = "")))){
      cat("Id: ", i, " -- PathRow: ", pathrowRaster, ", image .tif EXISTS in the folder\n", sep = "")
      next
    } else{
      ...
      if(isFALSE(landsat_different_exist) && isTRUE(pathrowRaster %in% pathrowShp)){
        ...
        rBrick <- raster::brick(raster.sub)

        ## save raster only for the shape that match with grid
        raster::writeRaster(rBrick, filename = paste(rasterFolder, newFolder, "/", filename, "_cutted.tif", sep = ""), format="GTiff", datatype = "INT1U", overwrite=TRUE)
      } else {
        cat("Id: ", i, " -- PathRow: ", pathrowRaster, ", image .tif provided by satellite different of the Landsat or off limits of the shapefile\n", sep = "")

        myRaster <- raster::brick(myRasters[i])
        myRaster[[3]][myRaster[[3]] == 255] <- 254 # change value to white color
        ## save raster only for the shape that match with grid
